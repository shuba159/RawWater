...
<head>
	...
	<script type="text/javascript">
		...
		function getQueDiv(strId){
			var str = "";
			$(".que_tr").each(function(){
				var che = $(this).find("td div").first();
				if($(che).hasClass('checked')){
					var td4 = $(this).find("td").eq(4);
					var td0 = $(this).find("td input").eq(0);
					var id = $(td0).attr('id');
					var test = $(td4).text();
					str = str + '<tr><td style="width: 35px"><input class="que_id" type="hidden" value="'+id+'"></td><td>'+test+'</td><td ><input type="text" readonly="readonly" class="form-control '+strId+' que_input"></td><td><button onclick="del_que(this)" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</button></td></tr>';
				}
			})
			return str;
		}
...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="ysQuestions" action="${ctx}/project/ysTest/quesionList/" method="post" class="form-inline">
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column questionType">题型</th>
				<th  class="sort-column questionState">状态</th>
				<th  class="sort-column describes">试题内容</th>
				<th  class="sort-column createId">创始人</th>
				<th  class="sort-column updateTime">修改时间</th>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="ysQuestions" varStatus="ysQuestionsList">
			<tr class="que_tr">
				<td> <input type="checkbox" id="${ysQuestions.id}" class="i-checks"></td>
				<td>
					${ysQuestionsList.index+1 }
				</td>
				<td>
				...
				<td>
					${ysQuestions.describes }&nbsp;&nbsp;&nbsp;${ysQuestions.options }
				</td>
				<td>
